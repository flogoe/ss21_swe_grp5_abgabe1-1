...
// Aufruf:   node-ts port-forward.ts film|mongodb|mailserver
...
switch (service) {
    case undefined:
    case 'film':
        exec(`kubectl port-forward service/film 3000 --namespace ${namespace}`);
        break;
    ...
    default:
        console.log('node-ts port-forward.ts film|mongodb|mailserver');
}
